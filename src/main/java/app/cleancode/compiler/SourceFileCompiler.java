                    ...
                    if (instruction.getType() == LabelNode.LABEL) {
                        ...
                        switch (instruction.getOpcode()) {
                            case Opcodes.GOTO: {
                                ...
                                String labelName =
                                        getOrCreateLabelName(numLabels, labelNames, target);
                            ...
                            case Opcodes.LDC: {
                                ...
                                sourceMethod.actions.add(
                                        new LoadConstantAction(ldcInstruction.cst, ldcOutputs));
                            ...
                            case Opcodes.PUTSTATIC: {
                                ...
                                sourceMethod.actions.add(new PutAction(sourceVariable,
                                        NameMangler.mangle(classNode.name, fieldInstruction.name,
                                                fieldInstruction.desc)));
                                operandStack.pop(sourceVariable);
                                sourceMethod.variables.add(new Pair<>(
                                        TypeHelper.getCStyleTypeName(fieldInstruction.desc),
                                        sourceVariable));
                            ...
                            case Opcodes.INVOKESPECIAL: {
                                ...
                                if (Type.getReturnType(methodInstruction.desc)
                                        .getSort() != Type.VOID) {
                                    operandStack.push(outputs,
                                            TypeHelper.getCStyleTypeName(
                                                    Type.getReturnType(methodInstruction.desc)
                                                            .getDescriptor()));
                                }
                                break;
                            }
                            ...
                            case Opcodes.AALOAD: {
                                ...
                                List<String> outputs = new ArrayList<>();
                                sourceMethod.actions.add(new ArrayIndexAction(arrayVariable,
                                        indexVariable, outputs));
                                String indexType = operandStack.pop(indexVariable);
                                String arrayType = operandStack.pop(arrayVariable);
                                operandStack.push(outputs,
                                        TypeHelper.removeDimensionCStyle(arrayType));
                                sourceMethod.variables
                            ...
                            case Opcodes.IF_ICMPLT: {
                                JumpInsnNode jumpInstruction = (JumpInsnNode) instruction;
                                String variableA =
                                        getTmpVariableName(sourceMethod.variables) + "_a";
                                String variableB =
                                        getTmpVariableName(sourceMethod.variables) + "_b";
                                sourceMethod.actions.add(new CompareAction(variableA, "<",
                                        variableB, getOrCreateLabelName(numLabels, labelNames,
                                                jumpInstruction.label.getLabel())));
